...
const HINGE_CONFIG: Record<string, HingeConfig> = {
...
export interface FoldTextProps {
  text?: string;
  splitBy?: 'char' | 'word' | 'line' | 'words' | 'chars';
...
export default function FoldText({
  ...
  const activeSplit: 'char' | 'word' | 'line' =
    splitBy === 'words' ? 'word' : splitBy === 'chars' ? 'char' : (splitBy || 'word');

  const segments = useMemo(() => {
    let segmentIndex = 0;

    const renderSegment = (content: React.ReactNode, key: string, split = activeSplit) => {
      segmentIndex += 1;
      return (
        <span
    ...
    if (activeSplit === 'word') {
      return displayText.split(/(\s+)/).flatMap((part, index) => {
        if (!part) return [];
  ...
  }, [displayText, activeSplit, hinge, hingeConfig.origin, safePerspective]);
  ...
  return (
    <span ref={rootRef} className={`fold-text ${className}`.trim()} style={rootStyle}>
      <span className="fold-text-sr-only">{displayText}</span>
